...
const STATE_COOKIE = "slurpy_oauth_facebook_state";
const VERIFIER_COOKIE = "slurpy_oauth_facebook_verifier";
const RETURN_TO_COOKIE = "slurpy_oauth_facebook_return_to";
...
export const GET: APIRoute = async ({ locals, request, cookies, redirect }) => {
  ...
  const callbackUrl = new URL("/auth/oauth/facebook/callback", url).toString();
  ...
  try {
    await locals.pb
      .collection("users")
      .authWithOAuth2Code("facebook", code, codeVerifier, callbackUrl);
