// J'ai utilisé les Astro Actions pour implémenter des fonctions sécurisées côté serveur pour valider et traiter les formulaires
// de manière robuste et centralisée. (Form de contact, auth, avis, profil, recettes...). Cela permet de bénéficier d'une validation
// d'entrée stricte avec Zod, et d'une gestion centralisée des erreurs et d'une protection contre les abus.

// Je n'ai pas forcément eu le temps de découper chaque action dans des fichiers séparés (auth.ts, recette.ts, avis.ts...), donc pour l'instant
// le fichier est un monolithe beaucoup trop gros
...
// ==========================================================================
// ACTIONS ASTRO: definitions par feature
// ==========================================================================
export const server = {
  // ------------------------------------------------------------------------
  // CONTACT & NEWSLETTER
  ...
  // Enregistre une demande newsletter via l'email de contact avec rate limit.
  ...
  // Envoie le formulaire de contact avec honeypot anti-bot et rate limit.
  ...
  // Authentifie un utilisateur puis renvoie une redirection interne securisee.
  login: defineAction({
    accept: "form",
    input: loginSchema,
    handler: async (input, context) => {
  ...
  // Cree un compte utilisateur, gere le conflit email, puis connecte automatiquement.
  ...
  // Cree un avis pour une recette en imposant une session utilisateur valide.
  ...
  // Supprime un avis uniquement si son auteur correspond a l'utilisateur connecte.
  ...
  // Met a jour le profil utilisateur connecte, avec gestion optionnelle de l'avatar.
  ...
  // Cree une recette complete puis rollback la recette si composition/etapes echouent.
  ...
  // Met a jour une recette existante du proprietaire et remplace composition/etapes.
  ...
  // Supprime une recette uniquement si l'utilisateur connecte en est proprietaire.
...
// ==========================================================================
// UTILS: securite, normalisation et anti-abus
// ==========================================================================

// Limite des tentatives pour les actions de contact afin d'eviter les abus.
const RATE_LIMIT_WINDOW_MS = 10 * 60 * 1000;
const RATE_LIMIT_MAX_REQUESTS = 5;
const requestsByIp = new Map<string, number[]>();

// Bloque les requetes excessives depuis une meme adresse IP sur la fenetre definie.
...
// Uniformise un email pour eviter les differences de casse et d'espaces.
function normalizeEmail(email: string): string {
  return email.trim().toLowerCase();
}

// Accepte uniquement un retour interne pour bloquer les open redirects.
...
// ==========================================================================
// POCKETBASE ERRORS: extraction et normalisation des erreurs
// ==========================================================================

// Recupere le message d'erreur lie au champ email dans une reponse PocketBase.
...
// Extrait le bloc data d'une erreur PocketBase, selon son format de reponse.
...
// Retourne le premier message de champ pertinent selon un ordre de priorite donne.
...
// Recupere un message d'erreur generique quel que soit le format PocketBase rencontre.
...
// Extrait le statut HTTP/code d'une erreur PocketBase quand il est disponible.
...
// Prepare une structure d'erreur exploitable pour des logs serveurs utiles.
...
// ==========================================================================
// HELPERS AUTH & RECETTES: securisation et normalisation metier
// ==========================================================================

// Verifie la session courante et renvoie l'identifiant utilisateur authentifie.
...
// Genere un slug simple et stable a partir d'un titre de recette.
...
// Construit un slug unique en ajoutant un suffixe horodate en cas de collision.
...
// Construit dynamiquement le payload de mise a jour recette avec les champs fournis.
...
// Parse un champ JSON attendu comme tableau et leve une erreur action si invalide.
...
// Normalise les variantes d'unites ingredients vers une ecriture coherente.
...
// Nettoie et filtre les lignes de composition invalides avant persistence.
...
// Nettoie et filtre les etapes vides pour ne garder que les informations utiles.
